...
class GetAreaInfo:
    ...
    def check_coordi_detail(self,
        ...
        for y in range(coordinate[1], int(coordinate[1]+mask_height*1.5)):  # 上→下
            # 画像からはみ出したら終了
            if y == color_img.shape[0]-1 or y == int(coordinate[1]+mask_height*1.5-1):
                if detect_flag == 0:
                    break
                elif y == color_img.shape[0]-1:
                    y_min = color_img.shape[0]-1
                    break
                elif y == int(coordinate[1]+mask_height*1.5-1):
                    y_min = int(coordinate[1]+mask_height*1.5-1)
                    break
                else:
        ...
        for y in range(y_max, coordinate[1]-mask_height, -1):  # 下→上
            ...
            if y == 0 or y == coordinate[1]-mask_height + 1:
                if detect_flag == 0:
                    ...
                    break
                elif y == coordinate[1]-mask_height + 1:
                    y_min = coordinate[1]-mask_height + 1
                    break
        ...
        for x in range(x_max, coordinate[0]-mask_width, -1):  # 右→左
            ...
            if x == 0 or x == coordinate[0]-mask_width + 1:
                if detect_flag == 0:
                    ...
                    break
                elif x <= coordinate[0]-mask_width+1:
                    x_min = coordinate[0]-mask_width + 1
                    break
    ...
    def get_area_info(self, isR=True) -> None:
        ...
        if circle_blue_coordi2 is not None:
            ...
            self.course_info_block[3, 0] = self.check_block_exist(
                circle_blue_coordi2, exist_block_develop_img)
            # 座標の追加
        ...
        if circle_green_coordi1 is not None:
            ...
            self.course_info_block[3, 3] = self.check_block_exist(
                circle_green_coordi1, exist_block_develop_img)
            # 座標の追加
        ...
        if circle_green_coordi2 is not None:
            ...
            self.course_info_block[3, 2] = self.check_block_exist(
                circle_green_coordi2, exist_block_develop_img)
            # 座標の追加
        ...
        # 赤ブロックがなかった場合
        if self.block_count_red == 0:  # [603 564 131 108]
            # ブロック下の色をチェック
            if self.develop:
                # coordi_circle
                save_path = os.path.join(self.save_dir_path, "search_red_block_locate.png")
                cv2.imwrite(save_path, coordi_circle)
